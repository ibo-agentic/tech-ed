"""
Extract NCTB SSC Accounting (হিসাববিজ্ঞান) chapters via OCR.

Verified against actual TOC (12 chapters). Book page X = PDF page X+5.
"""
import os
import re
import sys
...
PDF_PATH = r"Secondary (BV)-2026_Class 9-10_Accounting_compressed.pdf"
OUTPUT_DIR = "books/ssc/accounting"
DPI = 200

# Verified page ranges — PDF page numbers (book page + 5 offset)
ACCOUNTING_PAGES = {
    "প্রথম":     (6,   12),   # Book ১-৭
    "দ্বিতীয়":   (13,  31),   # Book ৮-২৬
    "তৃতীয়":    (32,  44),   # Book ২৭-৩৯
    "চতুর্থ":    (45,  55),   # Book ৪০-৫০
    "পঞ্চম":     (56,  66),   # Book ৫১-৬১
    "ষষ্ঠ":      (67,  86),   # Book ৬২-৮১
    "সপ্তম":    (87,  111),  # Book ৮২-১০৬
    "অষ্টম":    (112, 133),  # Book ১০৭-১২৮
    "নবম":      (134, 148),  # Book ১২৯-১৪৩
    "দশম":      (149, 184),  # Book ১৪৪-১৭৯
    "একাদশ":   (185, 199),  # Book ১৮০-১৯৪
    "দ্বাদশ":    (200, 218),  # Book ১৯৫-২১৩
}
...
def chunk_text(text, chapter_label, max_words=400):
    ...
    for i in range(0, len(words), max_words):
        ...
        labeled = f"[হিসাববিজ্ঞান | {chapter_label}]\n\n{chunk}"
        chunks.append(labeled)
    return chunks


def extract_chapter(ordinal_key, start_page, end_page):
    chapter_number, chapter_title = CHAPTERS["accounting"][ordinal_key]
...
def main():
    print("=" * 60)
    print("NCTB Accounting (হিসাববিজ্ঞান) OCR Extraction")
    print("=" * 60)
    ...
    total = 0
    for ordinal_key, (start, end) in ACCOUNTING_PAGES.items():
        if ordinal_key not in CHAPTERS["accounting"]:
    ...
    print("\nNext steps:")
    print("  1. python rag/embed.py       (auto-detects new accounting subject)")
    print("  2. Add accounting card to landing page UI")
    print("  3. Test with theory question first, then math question")
